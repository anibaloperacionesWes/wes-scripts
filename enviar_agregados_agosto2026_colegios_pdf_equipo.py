"""
Convierte a PDF y sube a Drive — agregados agosto 2026 (lote colegios).

Drive: Agente WES / wes-scripts / reports / <cliente> / ABREGADO

Uso:
  python enviar_agregados_agosto2026_colegios_pdf_equipo.py
  python enviar_agregados_agosto2026_colegios_pdf_equipo.py --con-correo
"""
...
ENVIOS = [
    (
        ...
        "Renca/ABREGADO",
        "Renca — Reporte agregado PDF — Agosto 2026 (colegios)",
        "Adjunto el reporte agregado (PDF) de los colegios de Renca "
        f"(Lo Velásquez, Cumbre Pte., Cumbre Ote. / ICCO) para el periodo {PERIODO_TXT}.\n",
    ),
    (
        ROOT / "reports" / "La_Florida" / "ABREGADO",
        f"Reporte_Agregado_*_{SUFIJO}.docx",
        "La_Florida/ABREGADO",
        "La Florida — Reporte agregado PDF — Agosto 2026",
    ...
    (
        ...
        f"Reporte_Agregado_*_{SUFIJO}.docx",
        "CORMUP/ABREGADO",
        "CORMUP Peñalolén — Reporte agregado PDF — Agosto 2026",
        "Adjunto el reporte agregado (PDF) de los colegios CORMUP / Peñalolén "
        f"(14 establecimientos) para el periodo {PERIODO_TXT}.\n",
    ),
]
...
def main() -> int:
    ...
    parser.add_argument(
        "--con-correo",
        action="store_true",
        help="Enviar PDF a Juan, Diego y Aníbal (por defecto solo Drive).",
    )
    args = parser.parse_args()

    pw = _smtp_password() if args.con_correo else ""
    if args.con_correo and not pw:
