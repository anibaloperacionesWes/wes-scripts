"""
Script para generar solo reportes individuales de La Florida (sin presentación).
Período por defecto: 01-01-2026 al 31-01-2026 (sobrescribible con --start-date / --end-date).
"""
...
def main():
    ap = argparse.ArgumentParser(description="Reportes Word por nodo — La Florida")
    ap.add_argument("--start-date", default=START_DATE, help="Inicio (dd/mm/aaaa o ISO)")
    ap.add_argument("--end-date", default=END_DATE, help="Fin (dd/mm/aaaa o ISO)")
    args_cli = ap.parse_args()
    start_date = args_cli.start_date
    end_date = args_cli.end_date

    print("=" * 70)
    print("  GENERACIÓN DE REPORTES INDIVIDUALES - LA FLORIDA")
    ...
    try:
        ...
        company_name = "La Florida"
    print()
    
    # Obtener todos los nodos
    print("[1/2] Obteniendo nodos de La Florida...")
    nodes = get_florida_nodes(COMPANY_ID)
    
    if not nodes:
        print("[ERROR] No se encontraron nodos de La Florida.")
