"""
Reporte Word agregado Parque Arauco — Mall El Bosque.

Incluye todos los puntos de El Bosque según el listado Excel (columna Mall, p. ej. Arauco El Bosque)
o, si no hay Excel, según get_mall_name_for_parque_arauco + API empresa 000025.

Excluye LISTADO_PA_IDS_EXCLUIDOS y, además, 000025-11 (Matriz principal 1° piso), dado de baja.
apply_exclusions=False: el agregado por mall respeta solo estos filtros (no exclusiones globales genéricas).

Salida:
  reports/Parque_Arauco/El_Bosque/ABREGADO/AGREGADO_<ts>/Reporte_Agregado_Parque_Arauco_<YYYYMMDD>_<YYYYMMDD>.docx

Ejemplos:
  python generar_pa_el_bosque_agregado_periodo.py
  python generar_pa_el_bosque_agregado_periodo.py --desde 02/02/2026 --hasta 31/03/2026
  python generar_pa_el_bosque_agregado_periodo.py --listado-xlsx "reports/Parque_Arauco/Reportes_agregados_reunion_abril_2026/listado_que_esta_instalado.xlsx"
"""
...
MALL_NOMBRE = "El Bosque"


def _es_el_bosque_texto(mall_cell: str) -> bool:
    s = (mall_cell or "").strip().lower()
    return "bosque" in s


def _nodos_el_bosque_desde_xlsx(path: Path) -> List[str]:
    ...
    try:
        ...
        for row in rows:
            ...
            if mall is None or nid is None:
                continue
            if not _es_el_bosque_texto(str(mall)):
                continue
            nid_s = str(nid).strip()
...
def _nodos_el_bosque_desde_api() -> List[str]:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Agregado Word PA El Bosque (todos los puntos del mall).")
    ...
    parser.add_argument("--solo-api", action="store_true", help="Ignorar Excel; solo mapeo/API El Bosque.")
    ...
    if args.solo_api or not path_xlsx.exists():
        if not args.solo_api and not path_xlsx.exists():
            print(f"[INFO] No se encontró Excel: {path_xlsx.resolve()} — usando API + mapeo El Bosque.")
        node_ids = _nodos_el_bosque_desde_api()
        fuente = f"API + mapeo {MALL_NOMBRE}"
    else:
        node_ids = _nodos_el_bosque_desde_xlsx(Path(path_xlsx))
    ...
    if not node_ids:
        print("[ERROR] No quedaron nodos El Bosque tras filtros. Revisa el Excel o el mapeo.")
        return 1

    print(f"[INFO] Fuente nodos: {fuente}")
    print(f"[INFO] Periodo: {args.desde} a {hasta}")
    print(f"[INFO] Nodos El Bosque ({len(node_ids)}): {', '.join(node_ids)}")
    ...
    print(f"[OK] Reporte agregado El Bosque: {out}")
    return 0
