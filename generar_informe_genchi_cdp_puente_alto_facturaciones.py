"""
Informe comparativo facturaciones Aguas Andinas — CDP Puente Alto (Genchi / Gendarmería).

Estructura:
  reports/Genchi/CDP Puente Alto/facturaciones/
  reports/Genchi/CDP Puente Alto/informe/

Ventana de análisis: ene-2025 a la fecha.
Excluye factura 8427884 (feb-2025, equipos off por licitación) y mes 2025-02 del desglose.

Uso:
  python generar_informe_genchi_cdp_puente_alto_facturaciones.py
"""
...
CDP_DIR = ROOT / "reports" / "Genchi" / "CDP Puente Alto"
FACTURAS_DIR = CDP_DIR / "facturaciones"
OUT_BASE = CDP_DIR / "informe"

FECHA_DESDE_ANALISIS = date(2025, 1, 1)
FECHA_RETIRO_WES = date(2025, 6, 28)
FECHA_FIN_CON_WES = date(2025, 6, 27)
FACTURA_EXCLUIDA = "8427884"
MESES_EXCLUIDOS = {"2025-02"}
MESES_PROYECCION_ANUAL = 12

CUENTA = "1008941-7"
MEDIDOR = "120715107"
CLIENTE = "Gendarmería de Chile — CDP Detención Preventiva Puente Alto (Unidad Genchi)"
...
def _word(
    ...
    p.add_run("Tras retiro del monitoreo WES (jul-2025 a mar-2026):\n").bold = True
    p.add_run(
...
def main() -> int:
    ...
    conclusiones = [
        ...
        f"Tras el retiro del monitoreo WES (jul-2025 a mar-2026) se facturaron {_m3_fmt(total_m3_sin)} "
        f"por {_fmt_clp(total_clp_sin)}; el sobrecosto respecto a la línea base WES fue "
        f"{_fmt_clp(sobrecosto_sin)}.",
        f"En la ventana ene-2025 a mar-2026 (sin feb anómalo) el total fue {_m3_fmt(total_m3_ventana)} "
    ...
    csv_mes = OUT_BASE / f"facturaciones_genchi_cdp_mensual_{ts}.csv"
    ...
    csv_path = OUT_BASE / f"facturaciones_genchi_cdp_{ts}.csv"
    ...
    docx = OUT_BASE / f"Informe_Genchi_CDP_Puente_Alto_facturaciones_{ts}.docx"
