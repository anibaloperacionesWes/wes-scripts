...
- "App" = consumo WES (API medidas) del nodo 000017-04 (Esc. Lo Velásquez, Renca)
- Facturaciones Aguas Andinas = consumo total m³ informado en cada factura PDF

Fuente de facturas:
  reports/Renca/Coparacion App con Aguas Andinas/Escuela lo Velzaquez Facturas/*.pdf

Salida:
  reports/Renca/Coparacion App con Aguas Andinas/reporte_comparacion_Lo_Velasquez/

Uso:
  python generar_reporte_comparacion_app_vs_facturaciones_lo_velazquez_000017_04.py
"""
...
NODE_ID = "000017-04"
NODE_NOMBRE = "Esc. Lo Velásquez (000017-04)"
EMPRESA = "Renca"
...
def _doc_establecer_metadatos(doc: Document, generado_en: datetime) -> None:
    ...
    cp.keywords = "WES; Aguas Andinas; Escuela Lo Velásquez; Renca; facturación; consumo"
    ...
    cp.author = "Script informe Esc. Lo Velásquez Renca"
...
def _generar_grafico_barras(out_png: Path, filas: list[dict]) -> None:
    ...
    with matplotlib.rc_context(rc):
        ...
        fig.suptitle(
            "Por factura: app WES (izquierda) vs facturación (derecha) — Esc. Lo Velásquez",
            fontsize=14,
            fontweight="600",
            color="#0f172a",
            y=0.97,
...
def _generar_grafico_totales(
    ...
    *,
    ...
    with matplotlib.rc_context(rc):
        ...
        fig.suptitle(
            "Total acumulado: App WES vs facturación en cuenta — Esc. Lo Velásquez",
            fontsize=14,
            fontweight="600",
            color="#0f172a",
            y=0.96,
...
def generar_reporte() -> tuple[Path, Path | None]:
    ...
    doc.add_paragraph(
        ...
        f"referencia ({MONITOREO_WES_DESDE.strftime('%d-%m-%Y')}) en adelante. Las diferencias entre facturación y "
        "consumo WES pueden deberse a alcance del medidor, rezagos de lectura, estimaciones de Aguas Andinas en "
        "ciclos sin medición completa en el punto, o a la configuración del período en la boleta."
    )
    doc.add_paragraph(
    ...
    doc.add_paragraph(
        ...
        f"{MONITOREO_WES_DESDE.strftime('%d-%m-%Y')}: etapa previa a la ventana de transición; interpretar con cuidado "
        "si hubo arranque de monitoreo o lecturas estimadas."
    ...
    img_barras = OUT_DIR / f"chart_barras_lo_velazquez_todos_{run_id}.png"
    ...
    def _grafico_categoria(titulo: str, filas_d: list[dict], sufijo: str) -> None:
        ...
        for r in copia:
            r.pop("lectura_fin", None)
        img = OUT_DIR / f"chart_barras_lo_velazquez_{sufijo}_{run_id}.png"
    ...
    img_tot = OUT_DIR / f"chart_barras_lo_velazquez_total_acumulado_{run_id}.png"
    ...
    out_docx = OUT_DIR / "Comparacion_App_vs_Facturaciones_Lo_Velasquez_Renca.docx"
    if out_docx.exists():
        try:
            out_docx.unlink()
        except OSError:
