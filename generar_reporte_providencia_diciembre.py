"""
Script para generar reporte de Providencia para diciembre 2025
...
def obtener_nodos_providencia():
    """
    Obtiene todos los nodos de Providencia.
    """
    print(f"[1/4] Obteniendo nodos de Providencia...")
    ...
    if not node_ids:
        print(f"[ERROR] No se encontraron nodos para Providencia")
        return []
    
    print(f"[OK] Total de nodos a procesar: {len(node_ids)}")
    return node_ids


def generar_reporte():
    """
    Genera el reporte agregado de Providencia.
    """
    print(f"[2/4] Generando reporte agregado...")
    print(f"  Periodo: {START_DATE} a {END_DATE}")
    
    # Obtener nodos de Providencia
    node_ids = obtener_nodos_providencia()
...
def enviar_correo(reporte_path: Path, destinatarios: list):
    ...
    msg["Subject"] = f"Reporte Agregado - Providencia - Diciembre 2025"
    
    # Crear cuerpo del mensaje
    cuerpo = f"""
Estimados Diego y Juan,

Se adjunta el reporte agregado de Providencia correspondiente al mes de diciembre 2025.

Detalles del reporte:
- Empresa: {company_name}
- Periodo: {START_DATE} a {END_DATE}

Este reporte contiene análisis detallado de consumo, alertas de consumo nocturno y métricas consolidadas de todos los puntos de monitoreo.
        ...
        if pdf_path and pdf_path.exists():
            with open(pdf_path, "rb") as f:
                ...
                adjunto.add_header(
                    "Content-Disposition",
                    "attachment",
                    filename=f"Reporte_Agregado_Providencia_Diciembre_2025.pdf"
                )
                msg.attach(adjunto)
            ...
            with open(reporte_path, "rb") as f:
                ...
                adjunto.add_header(
                    "Content-Disposition",
                    "attachment",
                    filename=f"Reporte_Agregado_Providencia_Diciembre_2025.docx"
                )
                msg.attach(adjunto)
    except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Genera reporte agregado de Providencia")
    ...
    print("=" * 70)
    print("  GENERACION DE REPORTE - PROVIDENCIA - DICIEMBRE 2025")
    print("=" * 70)
    print("")
