"""
Script para generar reporte agregado de Parque Arauco El Bosque y PPT
Período: 15 de diciembre 2025 al 20 de enero 2026
"""
...
# Fechas: desde 15 de diciembre 2025 al 19 de enero 2026
START_DATE = "15/12/2025"
END_DATE = "19/01/2026"

def get_el_bosque_nodes(company_id: str) -> list:
    """Obtiene los nodos de Parque Arauco El Bosque."""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for node in all_nodes:
                ...
                if mall_name == "El Bosque":
                    el_bosque_nodes.append({
                        "nodeId": node_id,
                        "name": node_name
                    })
            return el_bosque_nodes
        return []
...
def main():
    print("=" * 70)
    print("  GENERACIÓN DE REPORTES Y PPT - PARQUE ARAUCO EL BOSQUE")
    print(f"  Período: {START_DATE} - {END_DATE}")
    print("=" * 70)
    print()
    
    # Obtener nodos de El Bosque
    print("[1/5] Obteniendo nodos de Parque Arauco El Bosque...")
    nodes = get_el_bosque_nodes(COMPANY_ID)
    
    if not nodes:
        print("[ERROR] No se encontraron nodos de Parque Arauco El Bosque.")
        return

    _inact = NODOS_INACTIVOS_POR_MALL.get("El Bosque", frozenset())
    _raw = list(nodes)
    ...
    if not nodes:
        print("[ERROR] No quedaron nodos El Bosque tras excluir inactivos.")
        return
    ...
    for i, node in enumerate(nodes, 1):
        ...
        try:
            args = argparse.Namespace(
                ...
                output_dir="reports",
                enviar_correo=False,
                mall_name="El Bosque"  # Especificar mall para estructura de carpetas
            )
    ...
    try:
        ...
        output_dir_base = Path("reports") / safe_company_name / "El_Bosque" / "ABREGADO"
        output_dir_base.mkdir(parents=True, exist_ok=True)
        ppt_dir = output_dir_base / f"AGREGADO_EL_BOSQUE_{timestamp}"
    ...
    try:
        ...
        crear_ppt_analisis(datos, ppt_path, mall_name="El Bosque")
