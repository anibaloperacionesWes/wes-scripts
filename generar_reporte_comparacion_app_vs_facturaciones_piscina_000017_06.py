...
- "App" = consumo WES (API medidas) del nodo 000017-06 (Piscina municipal, Renca)
- Facturaciones Aguas Andinas = consumo total m³ informado en cada factura PDF (o filas CSV)

Fuentes (carpeta ``Piscina Facturaciones``), en este orden:
...
Salida (todo en ``reporte_comparacion_Piscina/``):
  - Comparacion_Facturacion_vs_WES_Piscina_Renca.xlsx (paso 1 del comando principal)
  - Comparacion_App_vs_Facturaciones_Piscina_Renca.docx y .pdf (tablas y gráficos usan el Excel si está disponible)
  - Gráficos ``chart_barras_piscina_*.png``

Uso:
  python generar_reporte_comparacion_app_vs_facturaciones_piscina_000017_06.py
"""
...
XLSX_INFORME = "Comparacion_Facturacion_vs_WES_Piscina_Renca.xlsx"
NODE_ID = "000017-06"
NODE_NOMBRE = "Piscina municipal (000017-06)"
EMPRESA = "Renca"
...
# Narrativa operacional (Piscina); cuadros comparativos usan INFORME_COMPARATIVO_DESDE (misma fecha aquí).
...
def _doc_establecer_metadatos(doc: Document, generado_en: datetime) -> None:
    ...
    cp.keywords = "WES; Aguas Andinas; Piscina municipal; Renca; facturación; consumo"
    ...
    cp.author = "Script informe Piscina municipal Renca"
...
def _cargar_facturas() -> list[FacturaAA]:
    ...
    if not pdfs:
        ...
        raise FileNotFoundError(
            ...
            "con una fila por factura (ver docstring del script generar_reporte_comparacion_app_vs_facturaciones_piscina_000017_06.py)."
            + extra
        )
    errores: list[str] = []
    rows: list[FacturaAA] = []
...
def _generar_grafico_barras(out_png: Path, filas: list[dict]) -> None:
    ...
    with matplotlib.rc_context(rc):
        ...
        fig.text(
            0.5,
            0.935,
            f"Piscina municipal · nodo {NODE_ID} — lecturas anterior y actual ≥ {INFORME_COMPARATIVO_DESDE.strftime('%d-%m-%Y')}",
            ha="center",
            fontsize=10,
            color="#64748b",
        )
...
def _generar_grafico_totales(
    ...
    *,
    ...
    with matplotlib.rc_context(rc):
        ...
        fig.suptitle(
            "Total acumulado: App WES vs facturación en cuenta — Piscina municipal",
            fontsize=14,
            fontweight="600",
            color="#0f172a",
            y=0.96,
...
def generar_reporte() -> tuple[Path, Path | None]:
    ...
    doc.add_paragraph(
        f"En la Piscina municipal (nodo {NODE_ID}) es frecuente que el consumo facturado por Aguas Andinas "
        ...
        "colectivos (otros usuarios o tramos de red compartida) antes de que el agua llegue al recinto o sector "
        "donde está instalado el punto monitoreado por WES. Por tanto, la facturación puede reflejar un caudal "
        "acumulado o repartido de forma distinta al flujo que atraviesa el ámbito del sensor en la piscina municipal; ello "
    ...
    p_a = doc.add_paragraph(
        ...
        "consumo de tramos compartidos antes del recinto de la piscina municipal (véase apartado 1)."
    )
    ...
    img_barras = OUT_DIR / f"chart_barras_piscina_todos_{run_id}.png"
    ...
    def _grafico_categoria(titulo: str, filas_d: list[dict], sufijo: str) -> None:
        ...
        for r in copia:
            r.pop("lectura_fin", None)
        img = OUT_DIR / f"chart_barras_piscina_{sufijo}_{run_id}.png"
    ...
    img_tot = OUT_DIR / f"chart_barras_piscina_total_acumulado_{run_id}.png"
    ...
    out_docx = OUT_DIR / "Comparacion_App_vs_Facturaciones_Piscina_Renca.docx"
    if out_docx.exists():
        try:
            out_docx.unlink()
        except OSError:
    ...
    try:
        ...
        if r.returncode != 0:
            print(
                ...
                "  Revise facturas/CSV y ejecute: python exportar_excel_comparacion_facturacion_vs_wes.py --site piscina",
                file=sys.stderr,
            )
        else:
