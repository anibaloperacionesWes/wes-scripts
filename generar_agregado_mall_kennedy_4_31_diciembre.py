"""
Script para generar el reporte agregado de Mall Kennedy 
desde el 4 de diciembre hasta el 31 de diciembre 2025.
No genera reportes individuales ni envía correos electrónicos.
"""
...
def main():
    print("=" * 60)
    print("  REPORTE AGREGADO MALL KENNEDY")
    print("  4 de Diciembre - 31 de Diciembre 2025")
    print("=" * 60)
    print()
    ...
    print(f"[1/2] Obteniendo nodos de {company_name}...")
    ...
    print(f"[2/3] Generando reporte agregado...")
    ...
    print()
    print(f"[3/3] Generando presentación PPT...")
    try:
        # Convertir formato de fecha si es necesario (YYYY-MM-DD a DD/MM/YYYY)
        start_date_formatted = START_DATE
        end_date_formatted = END_DATE
        if len(START_DATE.split('-')) == 3:  # Formato YYYY-MM-DD
            start_parts = START_DATE.split('-')
            start_date_formatted = f"{start_parts[2]}/{start_parts[1]}/{start_parts[0]}"
        if len(END_DATE.split('-')) == 3:  # Formato YYYY-MM-DD
            end_parts = END_DATE.split('-')
            end_date_formatted = f"{end_parts[2]}/{end_parts[1]}/{end_parts[0]}"
    ...
    print(f"[INFO] Ruta del reporte: {aggregated_report_path}")
    if 'ppt_path' in locals():
        print(f"[INFO] Ruta del PPT: {ppt_path}")
