"""
Reporte Word agregado Parque Arauco — Mall Estación.

Incluye todos los puntos de Estación según el listado Excel (columna Mall, p. ej. Arauco Estación)
o, si no hay Excel, según get_mall_name_for_parque_arauco + API empresa 000025.

Excluye LISTADO_PA_IDS_EXCLUIDOS (p. ej. 000025-02, 03, 05, 06 fuera de operación en listado).

Salida:
  reports/Parque_Arauco/Estación/ABREGADO/AGREGADO_<ts>/Reporte_Agregado_Parque_Arauco_<YYYYMMDD>_<YYYYMMDD>.docx

Ejemplos:
  python generar_pa_estacion_agregado_periodo.py
  python generar_pa_estacion_agregado_periodo.py --desde 02/02/2026 --hasta 31/03/2026
"""
...
MALL_NOMBRE = "Estación"


def _es_estacion_texto(mall_cell: str) -> bool:
    s = (mall_cell or "").strip().lower()
    return "estación" in s or "estacion" in s


def _nodos_estacion_desde_xlsx(path: Path) -> List[str]:
    ...
    try:
        ...
        for row in rows:
            ...
            if mall is None or nid is None:
                continue
            if not _es_estacion_texto(str(mall)):
                continue
            nid_s = str(nid).strip()
...
def _nodos_estacion_desde_api() -> List[str]:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Agregado Word PA Estación (todos los puntos del mall).")
    ...
    parser.add_argument("--solo-api", action="store_true", help="Ignorar Excel; solo mapeo/API Estación.")
    ...
    if args.solo_api or not path_xlsx.exists():
        if not args.solo_api and not path_xlsx.exists():
            print(f"[INFO] No se encontró Excel: {path_xlsx.resolve()} — usando API + mapeo Estación.")
        node_ids = _nodos_estacion_desde_api()
        fuente = f"API + mapeo {MALL_NOMBRE}"
    else:
        node_ids = _nodos_estacion_desde_xlsx(Path(path_xlsx))
    ...
    if not node_ids:
        print("[ERROR] No quedaron nodos Estación tras filtros. Revisa el Excel o el mapeo.")
        return 1

    print(f"[INFO] Fuente nodos: {fuente}")
    print(f"[INFO] Periodo: {args.desde} a {hasta}")
    print(f"[INFO] Nodos Estación ({len(node_ids)}): {', '.join(node_ids)}")
    ...
    print(f"[OK] Reporte agregado Estación: {out}")
    return 0
