"""
Reporte agregado DERCO — mismo formato que Fundo Zapallar completo:

  - Consumo total (suma de puntos activos)
  - Consumo diario por punto (sin marcadores de alerta)
  - Consumo nocturno 00:00–06:59 + barras amarillas
  - Comparativo del mes (nocturno vs efectivo)
  - Comparativo últimos 6 meses

Uso:
  python generar_agregado_derco_completo.py
  python generar_agregado_derco_completo.py --start-date 01/07/2026 --end-date 30/07/2026
"""
...
def _plot_barras_periodo(nodes: List[dict], out: Path) -> Path:
    ...
    ax.set_title("Consumo total del periodo por punto — DERCO", fontsize=13, fontweight="bold")
...
def _plot_6_meses(series: List[Tuple[str, float]], out: Path) -> Path:
    ...
    ax.set_title(
        "Comparativo últimos 6 meses — DERCO (suma puntos activos)",
        fontsize=12,
        fontweight="bold",
    )
...
def generar(start_date: str, end_date: str) -> Path:
    ...
    node_ids = _nodos_derco()
    if not node_ids:
        raise RuntimeError("No hay nodos DERCO tras exclusiones.")

    print("=" * 70)
    print(f"AGREGADO DERCO COMPLETO — {company_name}")
    ...
    out_dir = Path("reports") / "DERCO" / "ABREGADO" / f"AGREGADO_{stamp}"
    ...
    p = doc.add_paragraph(
        f"DERCO registró un consumo total de {format_number_chilean(total_m3, 1)} m³ "
        f"en el periodo, sumando los {len(nodes)} puntos activos "
        f"(se excluyen Lo Boza y puntos dados de baja)."
    )
    ...
    for n in nodes:
        ...
        if not measures:
            continue
        chart_path = out_dir / f"derco_diario_{n['node_id'].replace('-', '_')}.png"
    ...
    p2b = doc.add_paragraph(
        f"Nocturno total DERCO: {format_number_chilean(noct_total, 1)} m³ "
    ...
    p6 = doc.add_paragraph(
        "Consumo mensual total DERCO (suma de puntos activos). "
    ...
    _add_table(doc, ["Mes", "Total DERCO (m³)"], rows_6, has_total_row=False)

    add_formatted_heading(doc, "Conclusión", level=1)
    concl = doc.add_paragraph(
        f"En el periodo, DERCO registró {format_number_chilean(total_m3, 1)} m³, "
        f"de los cuales {format_number_chilean(noct_total, 1)} m³ correspondieron a "
        f"horario nocturno. Se recomienda seguir la evolución mensual para detectar desvíos."
    )
    concl.alignment = WD_PARAGRAPH_ALIGNMENT.JUSTIFY

    fname = f"Reporte_Agregado_DERCO_{start_dt:%Y%m%d}_{end_dt:%Y%m%d}.docx"
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(description="Agregado DERCO completo (formato FZ)")
