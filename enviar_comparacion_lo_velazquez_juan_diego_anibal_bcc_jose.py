"""
Envía comparación Esc. Lo Velásquez Renca (Word + PDF + Excel) a Juan, Diego y Aníbal.
...
def main() -> int:
    ...
    print("[1/3] Regenerando Excel Esc. Lo Velásquez…")
    r = subprocess.run(
        [sys.executable, str(ROOT / "exportar_excel_comparacion_facturacion_vs_wes.py"), "--site", "lo_velazquez"],
        cwd=str(ROOT),
    )
    if r.returncode != 0:
    ...
    docx = OUT_DIR / "Comparacion_App_vs_Facturaciones_Lo_Velasquez_Renca.docx"
    pdf = OUT_DIR / "Comparacion_App_vs_Facturaciones_Lo_Velasquez_Renca.pdf"
    xlsx = OUT_DIR / "Comparacion_Facturacion_vs_WES_Lo_Velasquez_Renca.xlsx"
    ...
    msg["Subject"] = "Esc. Lo Velásquez Renca — Comparación App WES vs facturaciones (Word + PDF + Excel)"
    msg.attach(
        MIMEText(
            "Estimados Juan, Diego y Aníbal,\n\n"
            "Adjunto el informe comparativo Escuela Lo Velásquez / Renca nodo 000017-04 (Word y PDF) y el Excel "
            "facturación vs WES (criterio de fechas según exportador; por defecto lecturas desde 26-08-2025).\n\n"
            "Saludos cordiales,\n"
            "Sistema WES\n",
            "plain",
            "utf-8",
