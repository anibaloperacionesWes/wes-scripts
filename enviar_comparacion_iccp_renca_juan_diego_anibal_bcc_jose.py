"""
Envía comparación ICCP Renca (Word + PDF + Excel) a Juan, Diego y Aníbal.
...
def main() -> int:
    ...
    print("[1/3] Regenerando Excel ICCP…")
    r = subprocess.run(
        [sys.executable, str(ROOT / "exportar_excel_comparacion_facturacion_vs_wes.py"), "--site", "iccp"],
        cwd=str(ROOT),
    )
    if r.returncode != 0:
    ...
    docx = OUT_DIR / "Comparacion_App_vs_Facturaciones_ICCP_Renca.docx"
    pdf = OUT_DIR / "Comparacion_App_vs_Facturaciones_ICCP_Renca.pdf"
    xlsx = OUT_DIR / "Comparacion_Facturacion_vs_WES_ICCP_Renca.xlsx"
    ...
    msg["Subject"] = "ICCP Renca — Comparación App WES vs facturaciones (Word + PDF + Excel)"
    msg.attach(
        MIMEText(
            "Estimados Juan, Diego y Aníbal,\n\n"
            "Adjunto el informe comparativo ICCP Renca (Word y PDF) y el Excel facturación vs WES "
            "(boletas con ambas lecturas desde el 26-08-2025, según criterio del informe).\n\n"
            "Saludos cordiales,\n"
            "Sistema WES\n",
            "plain",
            "utf-8",
    ...
    for p, subtype in ((docx, "vnd.openxmlformats-officedocument.wordprocessingml.document"), (pdf, "pdf"), (xlsx, "vnd.openxmlformats-officedocument.spreadsheetml.sheet")):
        with open(p, "rb") as f:
