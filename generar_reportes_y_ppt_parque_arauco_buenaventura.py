"""
Script para generar reporte agregado de Parque Arauco Buenaventura y PPT
...
def get_buenaventura_nodes(company_id: str) -> list:
    """Obtiene los nodos de Parque Arauco Buenaventura."""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for node in all_nodes:
                ...
                if mall_name == "Buenaventura":
                    buenaventura_nodes.append({
                        "nodeId": node_id,
                        "name": node_name
                    })
            return buenaventura_nodes
        return []
...
def main():
    print("=" * 70)
    print("  GENERACIÓN DE REPORTES Y PPT - PARQUE ARAUCO BUENAVENTURA")
    print(f"  Período: {START_DATE} - {END_DATE}")
    print("=" * 70)
    print()
    
    # Obtener nodos de Buenaventura
    print("[1/5] Obteniendo nodos de Parque Arauco Buenaventura...")
    nodes = get_buenaventura_nodes(COMPANY_ID)
    
    if not nodes:
        print("[ERROR] No se encontraron nodos de Parque Arauco Buenaventura.")
        return
    ...
    for i, node in enumerate(nodes, 1):
        ...
        try:
            args = argparse.Namespace(
                ...
                output_dir="reports",
                enviar_correo=False,
                mall_name="Buenaventura"  # Especificar mall para estructura de carpetas
            )
    ...
    try:
        ...
        output_dir_base = Path("reports") / safe_company_name / "Buenaventura" / "ABREGADO"
        output_dir_base.mkdir(parents=True, exist_ok=True)
        ppt_dir = output_dir_base / f"AGREGADO_BUENAVENTURA_{timestamp}"
    ...
    try:
        ...
        crear_ppt_analisis(datos, ppt_path, mall_name="Buenaventura")
