"""
Reporte agregado Nido de Águilas — formato tipo DERCO/FZ:

  - Consumo total + desagregado diario por punto (sin alertas)
  - Consumo nocturno 00:00–06:59 + barras amarillas
  - Comparativo del mes + últimos 6 meses
  - Notas operativas: Elementary (medidor) y Nido Piscina (alimentación)

Uso:
  python generar_agregado_nido_completo.py
  python generar_agregado_nido_completo.py --start-date 01/07/2026 --end-date 30/07/2026
"""
...
COMPANY_ID = "000007"
COMPANY_FOLDER = "Nido_de_Aguilas"
...
def _nodos() -> List[str]:
    ...
    return filter_node_ids(ids, company_id=COMPANY_ID, company_name="Nido de Aguilas")
...
def _plot_barras_periodo(nodes: List[dict], out: Path) -> Path:
    ...
    ax.set_title(
        "Consumo total del periodo por punto — Nido de Águilas",
        fontsize=13,
        fontweight="bold",
    )
...
def _plot_nocturno_amarillo(nodes: List[dict], out: Path) -> Path:
    ...
    ax.set_title(
        "Consumo nocturno por punto (00:00–06:59)",
        fontsize=13,
        fontweight="bold",
    )
...
def _plot_pareto(nodes: List[dict], out: Path) -> Path:
    pairs = sorted(
        ((n["node_name"], float(n["summary"]["total"])) for n in nodes),
        key=lambda x: -x[1],
    )
    ...
    ax1.set_ylabel("Consumo periodo (m³)", fontsize=10)
    ax1.set_xlabel("Punto, orden por consumo", fontsize=10)
    ax1.set_xticks(list(x))
    ax1.set_xticklabels(labels, rotation=35, ha="right", fontsize=9)
    ax1.set_title(
        "Pareto — uso del agua por punto (Nido de Águilas)",
        fontsize=12,
        fontweight="bold",
        pad=10,
    )
    ...
    ax2 = ax1.twinx()
    ax2.plot(x, cum, color=COLOR_PARETO, marker="o", linewidth=2, markersize=5)
    ax2.set_ylabel("% acumulado", fontsize=10)
    ax2.set_ylim(0, 105)
    ax2.axhline(80, color=COLOR_PARETO, linestyle="--", linewidth=1, alpha=0.55)
...
def _plot_6_meses(series: List[Tuple[str, float]], out: Path) -> Path:
    ...
    ax.set_title(
        "Comparativo últimos 6 meses — Nido de Águilas",
        fontsize=12,
        fontweight="bold",
    )
...
def generar(start_date: str, end_date: str) -> Path:
    ...
    node_ids = _nodos()
    if not node_ids:
        raise RuntimeError("No hay nodos Nido de Águilas tras exclusiones.")

    print("=" * 70)
    print(f"AGREGADO NIDO COMPLETO — {company_name}")
    print(f"Periodo: {start_date} → {end_date}")
    print(f"Nodos ({len(node_ids)}): {', '.join(node_ids)}")
    print("=" * 70)
    ...
    print("[INFO] Descargando últimos 6 meses (suma puntos)...", flush=True)
    ...
    sub = doc.add_paragraph(
        f"MONITOREO WES\n"
        f"Análisis consolidado de {len(nodes)} puntos de monitoreo\n"
    ...
    p = doc.add_paragraph(
        f"Nido de Águilas registró un consumo total de {format_number_chilean(total_m3, 1)} m³ "
        f"en el periodo, sumando los {len(nodes)} puntos activos "
        f"(se excluyen Nido Cancha y Control Nido de Águilas)."
    )
    p.alignment = WD_PARAGRAPH_ALIGNMENT.JUSTIFY
    add_picture_with_pagination(doc, str(chart_barras), Inches(6), keep_with_next=True)

    add_formatted_heading(doc, "1.1 Desagregado — consumo diario por punto", level=1)
    ...
    for n in nodes:
        ...
        if not measures:
            continue
        chart_path = out_dir / f"nido_diario_{n['node_id'].replace('-', '_')}.png"
        ...
        if n["node_id"] == "000007-04":
            nota = doc.add_paragraph(
                "Observación operativa — Elementary: se requiere revisión del medidor, "
                "dado que no está girando. Conviene verificar en terreno el estado del "
                "equipo y la transmisión de lecturas."
            )
            nota.alignment = WD_PARAGRAPH_ALIGNMENT.JUSTIFY
            for run in nota.runs:
                run.italic = True
        elif n["node_id"] == "000007-05":
            nota = doc.add_paragraph(
                "Observación operativa — Nido Piscina: se necesita confirmar si este "
                "medidor aún alimenta la piscina. En una prueba realizada con Eduardo "
                "Rivas se registró consumo, pero no se ha vuelto a observar consumo "
                "posteriormente."
            )
            nota.alignment = WD_PARAGRAPH_ALIGNMENT.JUSTIFY
            for run in nota.runs:
    ...
    p2b = doc.add_paragraph(
        f"Nocturno total: {format_number_chilean(noct_total, 1)} m³ "
    ...
    add_formatted_heading(doc, "3. Comparativo del mes (nocturno vs efectivo)", level=1)
    ...
    add_formatted_heading(doc, "4. Comparativo últimos 6 meses", level=1)
    p6 = doc.add_paragraph(
        "Consumo mensual total (suma de puntos activos). "
    ...
    _add_table(doc, ["Mes", "Total (m³)"], rows_6, has_total_row=False)

    add_formatted_heading(doc, "Conclusión", level=1)
    concl = doc.add_paragraph(
        f"En el periodo, Nido de Águilas registró {format_number_chilean(total_m3, 1)} m³, "
        f"de los cuales {format_number_chilean(noct_total, 1)} m³ correspondieron a "
        f"horario nocturno. Se recomienda seguir la evolución mensual y atender las "
        f"observaciones operativas de Elementary (medidor que no gira) y Nido Piscina "
        f"(confirmar si aún alimenta la piscina)."
    )
    concl.alignment = WD_PARAGRAPH_ALIGNMENT.JUSTIFY

    fname = f"Reporte_Agregado_Nido_de_Aguilas_{start_dt:%Y%m%d}_{end_dt:%Y%m%d}.docx"
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(description="Agregado Nido de Águilas completo")
